...
export default function CryptoMarketCapPage() {
  ...
  const rawMarketCap = stock.marketCap 
    ? (typeof stock.marketCap === 'number' ? stock.marketCap : parseFloat(stock.marketCap.replace(/[^0-9.]/g, ''))) 
    : generateMockMarketCap(stock.symbol);
  
  const currentCap = stock.marketCap || formatMarketCap(rawMarketCap);

  // Use raw value for history calculations
  const historyData = [
      { year: '2020', value: (rawMarketCap * 0.2).toFixed(2) },
      { year: '2021', value: (rawMarketCap * 0.8).toFixed(2) },
      { year: '2022', value: (rawMarketCap * 0.5).toFixed(2) },
      { year: '2023', value: (rawMarketCap * 0.9).toFixed(2) },
      { year: '2024', value: rawMarketCap.toFixed(2) },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="col-span-1 lg:col-span-8">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-8 leading-relaxed text-lg">
                    {stock.name} has a market capitalization of {currentCap}. Market capitalization in crypto refers to the total dollar market value of a cryptocurrency's circulating supply. It is calculated by multiplying the total number of coins in circulation by the current market price of one coin.
                </p>
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600 dark:text-gray-400 leading-relaxed mb-4">
                                Market capitalization is one of the most important metrics for determining the size of a cryptocurrency and its dominance in the market.
                            </p>
                            <ul className="list-disc list-inside text-gray-600 dark:text-gray-400 space-y-2">
                                <li><strong>Large-cap ($10B+):</strong> Bitcoin, Ethereum, mostly stable.</li>
                                <li><strong>Mid-cap ($1B - $10B):</strong> Potential growth with moderate risk.</li>
                                <li><strong>Small-cap (&lt;$1B):</strong> High validation risk and high volatility.</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div className="col-span-1 lg:col-span-4 space-y-6">
                ...
                <div className="bg-gray-50 dark:bg-zinc-900 rounded-xl p-6 border border-gray-200 dark:border-zinc-800">
                 ...
                 <div className="border border-gray-200 dark:border-zinc-800 rounded-xl p-6">
                    <h3 className="text-lg font-bold mb-4">Top Assets by Market Cap</h3>
                     <div className="space-y-4">
                        {cryptos.slice(0, 5).map((s, i) => (
                            <Link key={i} href={`/crypto/${s.symbol}/market-cap`} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-xs text-gray-500">{s.marketCap || formatMarketCap(generateMockMarketCap(s.symbol))}</span>
                                    </div>
                                </div>
                            </Link>
                        ))}
