...
export default function EtfsPage() {
  const { etfs, loading, searchQuery, setSearchQuery, currentPage, setCurrentPage, loadEtfs } = useEtf();
  const { user } = useAuth();
  ...
  // Filter ETFs based on search query
  ...
  const displayedEtfs = filteredEtfs.slice(startIndex, startIndex + ITEMS_PER_PAGE);
      ...
      <section className="mx-auto max-w-7xl px-6 lg:px-12 py-12">
        <div className="flex flex-col-reverse lg:flex-row items-center justify-between gap-12">
            <div className="max-w-xl">
                <h1 className="text-5xl font-bold mb-6 anta-regular tracking-tight">Invest in ETFs</h1>
                <p className="text-gray-600 dark:text-gray-400 mb-8 text-lg leading-relaxed">
                    Diversify your portfolio with Exchange Traded Funds. 
                    Track and invest in top ETFs from around the world.
                </p>
            ...
            <div className="flex-1">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                    <div>
                        ...
                        <h2 className="text-4xl font-bold anta-regular">Popular ETFs</h2>
                    </div>
                    ...
                    <div className="min-w-[500px]">
                        ...
                        <div className="space-y-2">
                            {loading ? (
                                ...
                                displayedEtfs.map((etf, i) => (
                                <Link href={`/etfs/${etf.symbol}`} key={i} className="group grid grid-cols-12 items-center p-4 hover:bg-gray-50 dark:hover:bg-zinc-900 rounded-xl transition-colors cursor-pointer">
                                    <div className="col-span-6 flex items-center gap-4">
                                        ...
                                        <div className="truncate">
                                            <div className="font-semibold text-gray-900 dark:text-white truncate">{etf.name}</div>
                                            <div className="text-xs text-gray-500 dark:text-gray-400 font-bold">{etf.symbol}</div>
                                        </div>
                                    </div>
                                    <div className="col-span-2 font-medium whitespace-nowrap">
                                        ${etf.price}
                                    </div>
                                    <div className={`col-span-2 text-sm font-medium whitespace-nowrap ${etf.isNegative ? 'text-red-500' : 'text-green-500'}`}>
                                        <div className="flex flex-col">
                                            <span>{etf.isNegative ? '↓' : '↑'} {etf.change}</span>
                                            <span className="text-xs text-gray-400 dark:text-gray-500">{etf.changeValue}</span>
                                        </div>
                                    </div>
                                ...
                                <div className="text-center py-10 text-gray-500">No ETFs found matching "{searchQuery}".</div>
                            )}
                        </div>
                    </div>
                </div>
                ...
                {/* Pagination Controls */}
                {!loading && filteredEtfs.length > ITEMS_PER_PAGE && (
