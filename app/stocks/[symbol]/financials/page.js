...
const incomeData = [
  ...
  { year: '2023', revenue: 383.9, netIncome: 96.9 },
  { year: 'TTM', revenue: 385.6, netIncome: 100.4 },
];
...
export default function FinancialsPage() {
  ...
  const financials = generatedData?.financials || [];
  const isPositive = !stock.isNegative;

  const renderTable = () => {
      // Mock rows based on tab, using generated financials where applicable
      let rows = [];
      ...
      if (activeTab === 'Income') {
          rows = [
              ...
              { name: 'Cost of Revenue', values: financials.map(f => `$${f.costOfRevenue}B`) },
              { name: 'Gross Profit', values: financials.map(f => `$${f.grossProfit}B`) },
              { name: 'Operating Expenses', values: financials.map(f => `$${f.opExpenses}B`) },
              { name: 'Net Income', values: financials.map(f => `$${f.netIncome}B`) },
          ];
      } else if (activeTab === 'Balance Sheet') {
          // Mocking balance sheet ratios based on revenue for consistency
           rows = [
              { name: 'Total Assets', values: financials.map(f => `$${(f.revenue * 0.85).toFixed(1)}B`) },
              { name: 'Total Liabilities', values: financials.map(f => `$${(f.revenue * 0.45).toFixed(1)}B`) },
              { name: 'Total Equity', values: financials.map(f => `$${(f.revenue * 0.40).toFixed(1)}B`) },
              { name: 'Cash & Equivalents', values: financials.map(f => `$${(f.revenue * 0.15).toFixed(1)}B`) },
              { name: 'Long Term Debt', values: financials.map(f => `$${(f.revenue * 0.25).toFixed(1)}B`) },
          ];
      } else {
          rows = [
              { name: 'Operating Cash Flow', values: financials.map(f => `$${(f.netIncome * 1.2).toFixed(1)}B`) },
              { name: 'Investing Cash Flow', values: financials.map(f => `-$${(f.revenue * 0.1).toFixed(1)}B`) },
              { name: 'Financing Cash Flow', values: financials.map(f => `-$${(f.revenue * 0.05).toFixed(1)}B`) },
              { name: 'Free Cash Flow', values: financials.map(f => `$${(f.netIncome * 0.9).toFixed(1)}B`) },
          ];
      }
