...
export default function StockNewsPage() {
  ...
  useEffect(() => {
    ...
    if (foundStock) {
      setStock(foundStock);
      // Generate news
      const gen = generateStockData(foundStock);
      setNews(gen.news);
      setLoading(false);
  ...
  useEffect(() => {
      if(stock && news.length === 0) {
          const gen = generateStockData(stock);
          setNews(gen.news);
      }
  }, [stock, news]);
                     ...
                     <div className="space-y-4">
                        {stocks.slice(0, 4).map((s, i) => (
                            <Link key={i} href={`/stocks/${s.symbol}`} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-zinc-800 flex items-center justify-center text-[10px] font-bold">
